...
class Simulation : public Solver, public Model
{
  ...
  {
    if(input_variable[iv["iGrainBoundaryBehaviour"]].getValue() == 2) return;
    ...
    // release-to-birth ratio with birth rate = 0
    if(sciantix_variable[sv["Xe133 released"]].getFinalValue())
