...
void InputReading( )
{
  ...
  Sciantix_options[16] = ReadOneSetting("idefective_fuel_rods", input_settings, input_check);
  Sciantix_options[17] = ReadOneSetting("ideviation_stoichiometry", input_settings, input_check);
  ...
  if (!input_initial_conditions.fail())
  {
    ...
    Sciantix_variables[55] = ReadOneParameter("Initial stoichiometry deviation", input_initial_conditions, input_check);
  }
