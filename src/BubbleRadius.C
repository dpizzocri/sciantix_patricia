...
double BubbleRadius(double temperature, double atoms_per_bubble)
{
	...
	switch(ibubble_radius)
	{
		...
		case 1 :
			// from [1], [3]
			bubble_radius = 1.0e-09 * pow(3.0*Covolume_vdw/(4.0*Pi),(1.0/3.0)) * pow(atoms_per_bubble,(1.0/3.0));
			break;

		case 2 :
			// from [2]
			bubble_radius = 1.0e-09 * pow(3.0*Cons_bolt*temperature/(8.0*Pi*Surface_tension),0.5) * pow(atoms_per_bubble,0.5);
			break;

		case 3 :
			// from [4]
			bubble_radius = 1.0e-09 * pow(3.0*0.4*Covolume_vdw/(4.0*Pi),(1.0/3.0)) * pow(atoms_per_bubble,(1.0/3.0));
			break;

		case 4 :
			// from [5]
			bubble_radius = 1.0e-09 * pow(3.0*0.6*Covolume_vdw/(4.0*Pi),(1.0/3.0)) * pow(atoms_per_bubble,(1.0/3.0));
			break;
