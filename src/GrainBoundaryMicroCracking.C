...
void GrainBoundaryMicroCracking( )
{
  ...
  const double inflection = 1773.0 + 520.0 * exp(- sciantix_variable[sv["Burnup"]].getFinalValue() /
    (10.0 * physics_constant[pc["U_UO2"]].getValue()));const double exponent = 33.0;
  ...
  // healing parameter
  const double healing_parameter = 1.0 / physics_constant[pc["U_UO2"]].getValue(); // 1 / (u * burnup)
