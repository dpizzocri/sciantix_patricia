...
void IntraGranularBubbleEvolution( )
{
  switch(iintra_bubble_evolution)
  {
    ...
    case 1 :
      {
        ...
        Intragranular_bubble_radius[0] = IntraGranularBubbleRadius(Temperature[1], Intragranular_atoms_per_bubble[0]);

        double resolution_rate = ResolutionRate(Intragranular_bubble_radius[0], Fissionrate[1]);
        double nucleation_rate = NucleationRate(Fissionrate[1]);
    ...
    case 2 :
      // from [2]
      Intragranular_bubble_concentration[1] = (1.52e+27 / Temperature[1]) - 3.3e+23;
      Intragranular_bubble_radius[1] = 5.0e-10 * (1.0 + 106.0 * exp(- 0.75 / (Cons_bolt * Temperature[1])));
      Intragranular_atoms_per_bubble[1] = Gas_grain_bubbles[1] / Intragranular_bubble_concentration[1];
      break;
